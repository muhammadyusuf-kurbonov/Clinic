    ...
    Assert.assertThrows(SuccessException::class.java) {
        runBlocking {
            try {
                withTimeout(timeout) {
                    collect {
                        println("received new value $it")
                        if (condition(it))
                            throw SuccessException()
                    }
                }
            } catch (e: TimeoutCancellationException) {
                throw IllegalStateException("Expected value wasn't received")
            }
        }
    }
}
...
val dummyCustomer: CustomerDTO = GsonBuilder().create()
    .fromJson(customerJson(), CustomerDTO::class.java)
